...
| Haiku agentic-query re-push of `claude-code-20250219`                           | query source                  | Not ported. §7.1, search `isAgenticQuery`. `kw` entry 1 suppresses the identifier for haiku and `cQt` re-pushes it when the query is agentic, so for a haiku model it appears after the `kw` run in agentic queries and not at all otherwise. The package's first push site is unconditional and takes no query-source input, so it can reproduce neither the presence rule nor the position.            |
