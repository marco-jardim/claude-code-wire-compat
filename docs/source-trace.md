...
a gap — while the new `claude-mythos-5-1` carries twelve capabilities but neither `per_turn_effort`
nor `refusal_fallback`, unlike the otherwise similar `claude-fable-5-1`. The eleven-capability entry
is `claude-fable-5`.
...
### 2.1.280 — recorded, not ported

Nine upstream behaviours observed while analysing 2.1.280 are recorded here so the next porter does not rediscover them as bugs. Section references are to `docs/protocol/versions/claude-code-2.1.280-analysis.md`; each row carries a search literal because that document's line numbers move whenever it is amended.

Two rows are deliberately **not** omissions, and say so. `L7t` is recorded because an absence that can never be observed is otherwise indistinguishable from a mistake, and `context_hint` is partly modelled rather than absent — calling either one "not ported" would be the kind of tidy summary this document exists to prevent.

| Upstream behaviour                                                              | Origin                        | Disposition                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Served capability through the remote client-data cache (`gq`, `nNn`)            | remote client-data cache      | Recorded, not ported. §7.8, search `Capability resolution has three layers`. `gq` can return true for a model the static catalogue does not declare, by reading the remote cache `nNn`. This package performs no remote read and cannot reproduce the lookup; at defaults that cache is empty, so the emitted header is unaffected.                                                                                                                                                                                                                                                                                                                                                                         |
| Permissive uncatalogued-model fallback in the mid-conversation-system predicate | upstream predicate tail       | Not ported. §13.3, search `return mD(sc(e))`. The predicate's last line grants `mid_conversation_system` to an uncatalogued model on any first-party-class provider. The package resolves the capability from the catalogue and denies an uncatalogued model instead of admitting it.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Haiku agentic-query re-push of `claude-code-20250219`                           | query source                  | Not ported. §7.1, search `isAgenticQuery`. `kw` entry 1 suppresses the identifier for haiku and `cQt` re-pushes it when the query is agentic, so for a haiku model it appears after the `kw` run in agentic queries and not at all otherwise. The package's first push site ports the `kw` entry 1 haiku suppression but takes no query-source input and has no re-push, so it can reproduce neither the presence rule nor the position: a haiku request built by this package omits `claude-code-20250219`, which the genuine client sends on the REPL main thread (`repl_main_thread` is agentic). `test/validation/shared-normalizer-ladder.test.ts` pins that omission as a known, recorded divergence. |
| `L7t` secondary 1M-context push                                                 | remote gate                   | **Not a divergence**, recorded so its absence is not mistaken for one. §7.6, search `L7t`. The push needs a remote string `kelp_forest_sonnet` that is absent by default, so it never fires for any model and a faithful client emits nothing here either.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `inline_tools` content blocks                                                   | conversation state            | Not ported. §11.1, search `inline_tools`. It is a conversation-state feature above the request-builder seam this package occupies. Whether it preserves a server-side prompt cache is not evidenced by the bundle, so even the motive is unconfirmed.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Per-turn `outputConfig` on `api_system` messages                                | turn ledger                   | Not ported. §11.2, search `Per-turn effort and timing`. The package builds one request rather than a turn ledger, so it holds no per-turn history to attach effort or timing to.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `lean_prompt` system-prompt selection                                           | system prompt authorship      | Not ported. §11.3, search `selects which system prompt is sent`. It selects which system prompt the client sends; this package authors no system prompts and forwards the caller's verbatim. It is a real input-token lever upstream, so its absence matters to a caller who expected the client to choose for them.                                                                                                                                                                                                                                                                                                                                                                                        |
| `clear_at` body construction                                                    | conversation state            | Not ported. §9.3, search `is on the wire`. The wire shape and the gate that emits the beta are recorded, but what `clear_at` does to billing or caching server-side is not evidenced by the bundle, and the value derives from conversation state this package does not hold.                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `context_hint` body field                                                       | per-request token measurement | **Partly modelled**, not a clean omission. §9.2, search `target_tokens_saved`. `src/request-body.ts` emits `context_hint: {enabled: true}` whenever the profile sets `contextHintEnabled`; the 2.1.280 profile sets it `false`, so this profile emits nothing. `target_tokens_saved` is not modelled at all, because its value comes from a per-request token measurement this package does not perform.                                                                                                                                                                                                                                                                                                    |
...
| Capability derivation                 | genuine client `Kw`, `Hke`, `Yte`, `Uot`, `QOt`, `RCn`, `LCn`, `U4e`   |           — | Predicates ported in `src/model-capabilities.ts` produce most booleans of `ClaudeCodeCapabilities`; since 2.1.280 two of them (`midConvToolChange`, `perTurnEffort`) come from catalogue strings that have no upstream predicate at all, so they are read from the catalogue and resolve `false` for an uncatalogued id. Each one reduces, on first party, to its exclusion list alone; the upstream `W9`/`JB`/`ZO` overrides and fallbacks are elided because no first-party path reaches them. `LCn` is the single allowlist — see "Temperature model gate".                                                                                                                                                                                                                                                                                                                                                   | `test/model-capability-wire.test.ts`        |
...
`ClaudeCodeCapabilities`, whose booleans are `thinking`, `adaptiveThinking`,
`interleavedThinking`, `effort`, `maxEffort`, `xhighEffort`, `contextManagement`, `temperature`,
`rejectsDisabledThinking` and, since 2.1.280, `midConvToolChange` and `perTurnEffort`.
...
- `betaOverrides.use1MContext` replaces ONLY the `[1m]` model-marker gate. The profile gate
  ...
  profile declares unavailable. It is a top-level field rather than an additional `ClaudeCodeCapabilities`
  key on purpose: capabilities are model-derived and cross-checked against `resolveModel()`, this
  gate is host state with no catalogue entry, and a mandatory key for host state in
  `evidence.capabilityDecisions` would change the evidence of every request without being
  derivable from the model. A model-derived capability may be added as a mandatory key -- 2.1.280
  added two, `midConvToolChange` and `perTurnEffort`, as a documented breaking change to the
  evidence type -- because `resolveModel()` can cross-check it. Host state cannot be cross-checked
  and so stays optional.
