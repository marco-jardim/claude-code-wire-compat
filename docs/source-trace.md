...
| Per-turn `outputConfig` on `api_system` messages                                | turn ledger                   | Not ported. §11.2, search `api_system.outputConfig`. The package builds one request rather than a turn ledger, so it holds no per-turn history to attach effort or timing to.                                                                                                                                                                                                                            |
| `lean_prompt` system-prompt selection                                           | system prompt authorship      | Not ported. §11.3, search `lean_prompt`. It selects which system prompt the client sends; this package authors no system prompts and forwards the caller's verbatim. It is a real input-token lever upstream, so its absence matters to a caller who expected the client to choose for them.                                                                                                             |
| `clear_at` body construction                                                    | conversation state            | Not ported. §9.3, search `clear_at`. The wire shape and the gate that emits the beta are recorded, but what `clear_at` does to billing or caching server-side is not evidenced by the bundle, and the value derives from conversation state this package does not hold.                                                                                                                                  |
