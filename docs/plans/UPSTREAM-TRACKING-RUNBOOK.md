...
7. **Fixtures and seal.** Add golden fixtures, run `npm run fixtures:seal`,
   ...
   `docs/plans/baseline-2026-08-05.md`, then confirm the seal with
   `npm run fixtures:check`. The sealed fixtures are the mechanical half of
   upstream tracking: this procedure is the only oracle for what upstream
   sends, and the seal is what stops a transcribed value from moving later
   without evidence.
